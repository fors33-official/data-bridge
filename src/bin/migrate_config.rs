...
//! 
//! Migrates legacy 2-field configs (price_field/volume_field) to N-field format
//! Also updates deprecated filter field names to new per-metric format
...
#[command(about = "Migrate legacy Data Bridge configs to N-field format")]
struct Cli {
    /// Path to legacy config file
    input: PathBuf,
    
    /// Path for migrated config (optional, defaults to input_migrated.toml)
    #[arg(short, long)]
...
fn main() {
    ...
    println!("Migrating config: {}", cli.input.display());
    
    // Read config
    let config_text = match fs::read_to_string(&cli.input) {
        Ok(text) => text,
        Err(e) => {
    ...
    // Parse TOML
    let mut config: toml::Value = match toml::from_str(&config_text) {
        Ok(cfg) => cfg,
        Err(e) => {
    ...
    // Migrate normalizer section
    if let Some(normalizer) = config.get_mut("normalizer").and_then(|n| n.as_table_mut()) {
        let price_field = normalizer.get("price_field").and_then(|v| v.as_str()).map(String::from);
        let volume_field = normalizer.get("volume_field").and_then(|v| v.as_str()).map(String::from);
        
        if let (Some(price), Some(volume)) = (price_field, volume_field) {
            // Add field_count
            normalizer.insert("field_count".to_string(), toml::Value::Integer(2));
            changes.push(format!("Added field_count = 2"));
            ...
            changes.push(format!("Created field_map: \"{}\" = 0, \"{}\" = 1", price, volume));
            
            // Comment out old fields (keep for reference)
            normalizer.remove("price_field");
            normalizer.remove("volume_field");
            changes.push("Removed price_field and volume_field".to_string());
            warnings.push("Legacy price_field/volume_field removed. Original values preserved in field_map.".to_string());
        }
    }
    
    // Migrate filter bounds
    if let Some(filter) = config.get_mut("filter").and_then(|f| f.as_table_mut()) {
        ...
        let volume_burst = filter.get("volume_burst_max_ratio").and_then(|v| v.as_float());
        ...
        if volume_burst.is_some() || burst_alpha.is_some() {
            ...
            if let Some(alpha) = burst_alpha {
                ...
                changes.push(format!("Migrated burst_ema_alpha to spike_detection.ema_alpha = {}", alpha));
            }
            
            if let Some(burst) = volume_burst {
                // Convert burst ratio to max_delta (approximate)
                spike_table.insert("metric_1_max_delta".to_string(), toml::Value::Float(burst * 100.0));
                changes.push(format!("Migrated volume_burst_max_ratio to spike_detection.metric_1_max_delta"));
            ...
            filter.insert("spike_detection".to_string(), toml::Value::Table(spike_table));
    ...
    // Update output headers if using legacy format
    if let Some(output) = config.get_mut("output").and_then(|o| o.as_table_mut()) {
        if !output.contains_key("headers") {
            let mut headers = Vec::new();
            headers.push(toml::Value::String("price".to_string()));
            headers.push(toml::Value::String("volume".to_string()));
    ...
    let migrated_text = toml::to_string_pretty(&config).expect("Failed to serialize TOML");
    ...
    if cli.dry_run {
        ...
        fs::write(&output_path, migrated_text).expect("Failed to write output file");
        println!("\n✅ Migrated config saved to: {}", output_path.display());
        println!("\nNext steps:");
        println!("1. Review the migrated config");
        println!("2. Test with: cargo run --release -- --config {}", output_path.display());
        println!("3. If successful, replace your original config");
    }
}
